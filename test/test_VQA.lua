...
--[[
for application on Visual Question Answering
...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Train a word-level Visual Question Answering model')
cmd:text()
cmd:text('Options')
-- data
cmd:option('-data_dir','data/7_version','data directory. Should contain the file input.txt with input data')
-- model params
...
cmd:option('-seq_length',7,'number of timesteps to unroll for')
...
if string.len(opt.init_from) > 0 then
    ...
    for c,i in pairs(checkpoint.vocab_Q) do 
        if not vocab_Q[c] == i then 
            vocab_compatible = false
        end
    end
    for c,i in pairs(checkpoint.vocab_A) do 
        if not vocab_A[c] == i then 
            vocab_compatible = false
        end
    end
    ...
    elseif opt.model == 'rnn' then
        protos.rnn = RNN.rnn(vocab_size_Q, opt.wordveclen, vocab_size_A, opt.rnn_size, opt.num_layers, opt.dropout)
    end
...
function eval_split(split_index, max_batches)
    ...
    for i = 1,n do -- iterate over batches in the split
        ...
        -- accuracy
        local conf,ind = torch.sort(prediction,2,true) -- last prediction
        --local pred = torch.DoubleTensor(opt.batch_size):copy(ind[{{},1}]) -- pred labels
        local pred = ind[{{},1}] -- pred labels
        accuracy = accuracy + (pred - y[{{}, opt.seq_length}]):eq(0):sum()
    ...
    accuracy = accuracy / n / opt.batch_size
...
for i = 1, iterations do
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        local train_loss, train_acc = eval_split(1) -- 1 = train
        local val_loss, val_acc = eval_split(2) -- 2 = validation
        local test_loss, test_acc = eval_split(3) -- 3 = test
        val_losses[i] = val_loss
        val_acces[i] = val_acc

        local savefile = string.format('%s/lm_%s_epoch%.2f_%.4f.t7', opt.checkpoint_dir, opt.savefile, epoch, val_loss)
        print('saving checkpoint to ' .. savefile)
        print(string.format('train accuracy: %.8f', train_acc))
        print(string.format('validation accuracy: %.8f', val_acc))
        print(string.format('test accuracy: %.8f', test_acc))
        local checkpoint = {}
        ...
        checkpoint.val_acces = val_acces
        checkpoint.i = i
        checkpoint.epoch = epoch
        checkpoint.vocab_Q = loader.vocab_mapping_Q
        checkpoint.vocab_A = loader.vocab_mapping_A
